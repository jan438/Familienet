...
def lookupimage(imgcode):
    ...
    newcalendar[42] = "kerstkaravaan"
    newcalendar[45] = "modeverkoop"
    newcalendar[46] = "draaiorgel"       
    newcalendar[47] = "bingo"  
    newcalendar[48] = "accordeon"  
    newcalendar[49] = "dagtour"
    newcalendar[50] = "gitaar"
    newcalendar[51] = "film"
    newcalendar[52] = "kaars"
    newcalendar[53] = "koor"
...
def drawActivity(c, activity_x, activity_y, w, h, a, i):
    ...
    if len(text) > 0:
        ...
        (first, rest, margin) = breakoff(inparts, descfontsize, activity_width - activity_angle)
        c.drawString(activity_x + 8.0 + 0.5 * margin, activity_y + activity_description_y, first)
        while len(rest) > 0:
...
os.chdir(path)
eventcal = "Calendar/calendar.ics"
...
lighteryellow = "#FEE773"
pinkredcircle = "#ff414e"
orangecircle = "#ff8928"
whitelayover = "#ffffff"
blacktext = "#000000"

c = Canvas("PDF/Aktiviteitenfam.pdf", pagesize=landscape(A4))
...
for i in range(0, len(monthevents)):
    ...
    if (i + 1) % 9 == 0 and (i + 1) < len(monthevents):
        c.showPage()
        c.setFillColor(HexColor(yellowbackground))
        c.rect(0, 0, A4_height, A4_width, fill = 1)
